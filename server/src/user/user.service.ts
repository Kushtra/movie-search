...
import { BadRequest, InternalServer, NotFound } from 'src/common/exceptions';
...
export class UserService {
  ...
  async create(user: User): Promise<number> {
    const id = await this.userRepository.nativeInsert(user).catch(err => {
      if (err?.code === UniqueViolation) throw new BadRequest('Email already in use');
      console.error(err);
  ...
  async fetchById(id: number): Promise<User> {
    const user = await this.userRepository.findOneOrFail({ id }).catch(err => {
      if (err instanceof NotFoundError) throw new NotFound('User not found');
      console.error(err);
      throw new InternalServer();
    });
